...
class GCN(nn.Module):
    def __init__(self, num_layer=5, emb_dim=300, feat_dim=256, drop_ratio=0, pool='mean'):
        ...
        #List of SAGPooling
        self.sag = nn.ModuleList()
        for layer in range(num_layer):
            self.sag.append(SAGPooling(emb_dim,0.5))

        # List of fc
        self.fc = nn.ModuleList()
        for layer in range(num_layer):
            self.fc.append(nn.Linear(emb_dim, emb_dim))

        # # List of relu
        # self.relu = nn.ModuleList()
        # for layer in range(num_layer):
        #     self.relu.append(F.relu(emb_dim))
        ...
        self.h_lin = nn.Linear(self.emb_dim, self.emb_dim)
    ...
    def forward(self, data):
        ...
        for layer in range(self.num_layer):
            ...
            # h =  SAGPooling(h)
            h1 = self.sag[layer](h,edge_index, edge_attr ,data.batch)
            h = h1[0]  # 22503*128
            edge_index = h1[1]
            edge_attr = h1[2]
            data.batch = h1[3]  # 22503
        ...
        h = self.pool(h, data.batch)#16*128
        # h = self.feat_lin(h)#16*512
        # out = nn.Linear(self.feat_dim, self.feat_dim * 2),
        out = self.out_lin(h)#16*256

        return h, out
        # return self.pool(h1[0], h1[3])
