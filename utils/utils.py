...
class Metrictor_DDI:
    def __init__(self, pre_y, truth_y, true_prob, is_binary=True):
        ...
        # print_file("TP: {}".format(self.TP))
        # print_file("TN: {}".format(self.TN))
        # print_file("FN: {}".format(self.FN))
        # print_file("FP: {}".format(self.FP))
        # print_file("总数为: {}".format(self.num))
    ...
    def show_result(self, is_print=False, file=None):
        ...
        self.specificity = self.TN / (self.TN + self.FP + 1e-10)
        ...
        if is_print:
            ...
            print_file("specificity: {}".format(self.specificity), file)
...
def get_bfs_sub_graph(ddi_list, node_num, node_to_edge_index, sub_graph_size):
    candiate_node = []
    selected_edge_index = []
    selected_node = []
    ...
    while len(selected_edge_index) < sub_graph_size:
        ...
        for edge_index in node_to_edge_index[cur_node]:
            ...
            if edge_index not in selected_edge_index:
                ...
                end_node = -1
                if ddi_list[edge_index][0] == cur_node:
                    end_node = ddi_list[edge_index][1]
                else:
                    end_node = ddi_list[edge_index][0]
...
def get_dfs_sub_graph(ddi_list, node_num, node_to_edge_index, sub_graph_size):
    stack = []
    selected_edge_index = []
    selected_node = []
